...
def solve_part_2(input):
    print(
        "For this actual puzzle, part 1 and 2 are resolved at the same time, please enter 1."
    )
...
class Bingo_board:
    def __init__(self, board, ind):
        self.board = board
        self.number = ind
        self.pinged_board = np.zeros(
            (5, 5), dtype=np.int8
        )  # default is one and turn to 0 if ping
        self.has_won = False
    ...
    def check_board(self):
        ...
        for axis in [0, 1]:
            ...
            index = np.where(
                np.sum(self.pinged_board, axis=axis) == self.pinged_board.shape[0]
            )[0]
    ...
    def calculate_board(self):
        print(
            "Board {} won and its calculation is : {}".format(
                self.number, np.sum(self.board, where=self.pinged_board == 0)
            )
        )
