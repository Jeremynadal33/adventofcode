...
def handle_main(bins):
    ...
    types = [type]
    print("type : ", type)
    _, number, versions, types, sub_numbers = handle_rest(
        6, type, bins, versions, types
    )
...
def handle_rest(index, type, file, versions, types, sub_numbers=[]):
    if type == 4:
        ...
        if file[index] == "0":
            ...
            while index < borne:
                index, to_add, versions, types, sub_numbers = handle_subpackets(
                    index, file, versions, types, sub_numbers
                )
                summ += to_add
