...
def solve_part_1(matrix, x1s, y1s, x2s, y2s ):
    ...
    for index in range(len(x1s)):
        matrix =  add_hor_vert_line(matrix, x1s[index], y1s[index], x2s[index], y2s[index]) 

    xs, ys = np.where(matrix>1) 
    print('There are {} points of at least 2 overlapping vertical/horizontal lines'.format(xs.shape[0]))
