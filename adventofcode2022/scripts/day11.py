...
def get_operation(line):
    ...
    if first == "old" and second == "old":
        ...
        return lambda x: operator(x, int(second))
    elif second == "old":
        return lambda x: operator(int(first), x)
    else:
        return lambda: operator(int(first), int(second))
...
class Monkey:
    def __init__(self, instructions):
        ...
        self.items_worry = [
            int(item) for item in instructions[1].strip().split(":")[1].split(",")
        ]
    ...
    def send_item(self, worry_drop):
        ...
        self.nb_inspected += 1
        if worry_drop:
            self.items_worry[0] = math.floor(self.operation(self.items_worry[0]) / 3)
        else:
            self.items_worry[0] = self.operation(self.items_worry[0])
        worry_item = self.items_worry.pop(0)
        index = (
            self.index_next_monkey_test_true
            if self.test(worry_item)
            else self.index_next_monkey_test_false
        )
