...
def get_graph(height_map, len_x, len_y):
    ...
    for x in range(len_x):
        for y in range(len_y):
            if height_map[x][y] == "S":
                start_coord = str_coord(x, y)
            if height_map[x][y] == "E":
                end_coord = str_coord(x, y)
            G.add_node(f"{x};{y}")
            if x == 0 and y == 0:  # en haut à gauche
                if should_add_edge(height_map, x, y, x + 1, y):
                    G.add_edge(str_coord(x, y), str_coord(x + 1, y))
                if char_to_num(height_map[x][y]) + 1 >= char_to_num(
                    height_map[x][y + 1]
                ):
                    G.add_edge(str_coord(x, y), str_coord(x, y + 1))
...
def solve_part_1(input_file):
    ...
    print(
        len(nx.shortest_path(G, start_coord, end_coord)) - 1
    )  # on enleve la première case car on veut le nbr de step
    print(short)
...
def solve_part_2(input_file):
    ...
    for init in start_coords:
        try:
            lengths.append(
                len(nx.shortest_path(G, init, end_coord)) - 1
            )  # on enleve la première case car on veut le nbr de step
        except:
