...
def handle_move(line, xh, yh, xt, yt, unique_positions):
    ...
    for _ in range(int(num)):
        ...
        if f"{xt}.{yt}" not in unique_positions:
            unique_positions.append(f"{xt}.{yt}")
        # print(xh, yh, xt, yt)
...
def handle_move_bis(line, knots, unique_positions):
    ...
    for _ in range(int(num)):
        ...
        for index in range(1, len(knots)):
            knots[index].x, knots[index].y = move_tail(
                knots[index - 1].x, knots[index - 1].y, knots[index].x, knots[index].y
            )
        if knots[-1].str_position() not in unique_positions:
            unique_positions.append(knots[-1].str_position())
        # print(xh, yh, xt, yt)
...
def solve_part_1(input_file):
    ...
    for line in file:
        # print(f'------------------{line}-----------------')
        xh, yh, xt, yt, unique_positions = handle_move(
            line, xh, yh, xt, yt, unique_positions
        )
        # print(xh, yh)
