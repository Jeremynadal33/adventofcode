...
def draw_pixel(X, cycle):
    if cycle == 40 and X in [39, 40, 41]: retu =  '#'
    elif abs(cycle%40 - X) <= 1 and cycle != 40: retu =  '#'
    else: retu =  '.'
...
def solve_part_1(input_file):
    ...
    for line in file:
        ...
        if should_store_register(cycle_count) : interesting.append(X)
...
def solve_part_2(input_file):
    ...
    for line in file:
        ...
        if len(intructions) == 1:
            if should_go_to_line(cycle_count) : output.write('\n')
