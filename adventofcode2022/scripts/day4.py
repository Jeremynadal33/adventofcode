...
def handle_line(line):
    ...
    elf_1 = range(int(line.split(',')[0].split('-')[0]), int(line.split(',')[0].split('-')[1]) + 1)
    elf_2 = range(int(line.split(',')[1].split('-')[0]), int(line.split(',')[1].split('-')[1]) + 1)
    return elf_1, elf_2

def is_pairs_fully_overlap(elf_1, elf_2):
    '''return boolean if one of the pairs is fully contained into the other'''
    return ( elf_1[0] in elf_2 and elf_1[-1] in elf_2) or (elf_2[0] in elf_1 and elf_2[-1] in elf_1)
...
def is_pairs_overlap(elf_1, elf_2):
    ...
    # print('---------')
    return not ( elf_1[-1] < elf_2[0] or elf_1[0] > elf_2[-1]) # and (elf_2[-1] <= elf_1[0] or elf_2[0] >= elf_1[-1]) )
