...
def create_inputs_files():
    ...
    if not os.path.exists(folder_name):
        logging.error(f"Folder {folder_name} does not exists. Are you running the script from the root ?")
        exit()
    if not os.path.exists(f"{folder_name}/inputs"): os.system(f"mkdir {folder_name}/inputs")
...
def create_bootstrap_script():
    ...
    if not os.path.exists(folder_name):
        logging.error(f"Folder {folder_name} does not exists. Are you running the script from the root ?")
        exit()
    if not os.path.exists(f"{folder_name}/scripts"): os.system(f"mkdir {folder_name}/scripts")
